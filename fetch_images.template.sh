#!/bin/bash

# --- Exit if any command fails ---
set -e

###############################################################################
# USAGE:
#   ./fetch_images.sh input.csv collection [output_folder] [--max-size-kb N]
#                    [--quality Q] [--resize-to WxH] [--column COL]
#                    [--tmp-dir DIR]
#
# Positional args:
#   $1 -> CSV path (required)
#   $2 -> collection name (required)
#   $3 -> output folder (optional)
#
# Additional resize/compression args can be passed as flags after $3.
###############################################################################

# --- Check for CSV argument ---
if [ -z "$1" ]; then
  echo "Usage: $0 path/to/input.csv collection_name [output_folder] [flags...]"
  exit 1
fi

# --- Check for collection argument ---
if [ -z "$2" ]; then
  echo "Error: Missing collection argument."
  echo "Usage: $0 path/to/input.csv collection_name [output_folder] [flags...]"
  exit 1
fi

CSV_PATH="$1"
COLLECTION="$2"
OUTPUT_FOLDER="${3:-utm_trs_images}"

# Shift first 3 args so remaining "$@" are forwarded to Python
shift 3 || true

# --- Export S3 environment variables ---
export S3_ENDPOINT="https://gateway:port"
export S3_BUCKET="bucket_name"
export S3_ACCESS_KEY="key_name"
export S3_PREFIX="prefix/"
export S3_SECRET_KEY="secret key"
export S3_URL_EXPIRY="3600"
export S3_REGION="s3_region"

echo "Using CSV: $CSV_PATH"
echo "Collection: $COLLECTION"
echo "Output folder: $OUTPUT_FOLDER"
echo "Passing extra args:" "$@"
echo "S3 environment variables loaded."
echo

# --- Run the Python script ---
python3 fetch_images.py \
  --csv "$CSV_PATH" \
  --collection "$COLLECTION" \
  --output "$OUTPUT_FOLDER" \
  "$@"

echo "Done."
